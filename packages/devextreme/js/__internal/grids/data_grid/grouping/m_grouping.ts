...
const dataSourceAdapterExtender = (Base: ModuleType<DataSourceAdapter>) =>
  class GroupingDataSourceAdapterExtender extends Base {
    ...
    protected itemsCount() {
      return this._dataSource.group()
        ...
        : super.itemsCount.apply(this, arguments as any);
    }
...
const GroupingDataControllerExtender = (Base: ModuleType<DataController>) =>
  class GroupingDataControllerExtender extends Base {
    ...
    protected _processItem(item, options) {
      if (
        isDefined(item.groupIndex) &&
        isString(item.rowType) &&
        item.rowType.indexOf("group") === 0
      ) {
      ...
      if (!options) {
        ...
        options = {
          collectContinuationItems:
            scrollingMode !== "virtual" && scrollingMode !== "infinite",
          resultItems: [],
          path: [],
          values: [],
        };
    ...
    protected _changeRowExpandCore(key) {
      ...
      if (!dataSource) {
        ...
        when(dataSource.changeRowExpand(key))
          .done(() => {
...
const allowDragging = (groupPanelOptions, column): boolean => {
  ...
  const canDrag =
    groupPanelOptions?.allowColumnDragging && column?.allowGrouping;

  return isVisible && !!canDrag;
};

export const GroupingHeaderPanelExtender = (Base: ModuleType<HeaderPanel>) =>
  class GroupingHeaderPanelExtender extends ColumnContextMenuMixin(Base) {
    ...
    private _handleActionKeyDown(args) {
      ...
      const groupColumnIndex = $target
        .closest(`.${CLASSES.groupPanelItem}`)
        .index();
      const column =
        this._columnsController.getGroupColumns()[groupColumnIndex];
    ...
    private getGroupAndUngroupItems(options) {
      ...
      if (contextMenuEnabled && column) {
        const isGroupingAllowed = isDefined(column.allowGrouping)
          ? column.allowGrouping
          : true;

        if (isGroupingAllowed) {
          const isColumnGrouped =
            isDefined(column.groupIndex) && column.groupIndex > -1;
    ...
    protected _columnOptionChanged(e?) {
      if (
        !this._requireReady &&
        !gridCore.checkChanges(e.optionNames, ["width", "visibleWidth"])
      ) {
    ...
    private _updateGroupPanelContent($groupPanel) {
      ...
      if (groupPanelOptions.allowColumnDragging && !groupColumns.length) {
        ...
        $groupPanel
          .closest(`.${CLASSES.groupPanelContainer}`)
          .addClass(CLASSES.groupPanelLabel);
        $groupPanel
          .closest(`.${CLASSES.groupPanelLabel}`)
          .css("maxWidth", "none");
      }
    }
    ...
    public getContextMenuItems(options) {
      ...
      const $groupedColumnElement = $(options.targetElement).closest(
        `.${CLASSES.groupPanelItem}`
      );
...
const GroupingRowsViewExtender = (Base: ModuleType<RowsView>) =>
  class GroupingRowsViewExtender extends Base {
    private getContextMenuItems(options) {
      ...
      if (
        contextMenuEnabled &&
        options.row &&
        options.row.rowType === "group"
      ) {
        const columnsController = that._columnsController;
        const column = columnsController.columnOption(
    ...
    protected _rowClick(e) {
      ...
      const scrollingMode = that.option("scrolling.mode");
      const isGroupRowStateChanged =
        scrollingMode !== "infinite" &&
        expandMode === "rowClick" &&
        $(e.event.target).closest(`.${DATAGRID_GROUP_ROW_CLASS}`).length;
      const isExpandButtonClicked = $(e.event.target).closest(
        `.${DATAGRID_EXPAND_CLASS}`
      ).length;
    ...
    private _changeGroupRowState(e) {
      const row = this._dataController.items()[e.rowIndex] as any;
      const allowCollapsing = this._columnsController.columnOption(
      ...
      if (
        row.rowType === "data" ||
        (row.rowType === "group" && allowCollapsing !== false)
      ) {
        // @ts-expect-error
...
const columnHeadersViewExtender = (Base: ModuleType<ColumnHeadersView>) =>
  class GroupingHeadersViewExtender extends Base {
    public getContextMenuItems(options) {
      ...
      if (
        contextMenuEnabled &&
        options.row &&
        (options.row.rowType === "header" ||
          options.row.rowType === "detailAdaptive")
      ) {
        const { column, rowIndex } = options;

        if (
          !column.command &&
          (!isDefined(column.allowGrouping) || column.allowGrouping)
        ) {
          const groupingTexts: any = that.option("grouping.texts");
          const isColumnGrouped =
            isDefined(column.groupIndex) && column.groupIndex > -1;
          const onItemClick = onGroupingMenuItemClick.bind(
            that,
            column,
            rowIndex
          );
      ...
      if (groupItems.length) {
        ...
        const clearSortingItemIndex =
          items.findIndex((item) => item.name === "clearSorting") + 1;
    ...
    public allowDragging(column): boolean {
      ...
      return (
        allowDragging(groupPanelOptions, column) || super.allowDragging(column)
      );
    }
  };
...
gridCore.registerModule("grouping", {
  defaultOptions() {
    return {
      grouping: {
        ...
        texts: {
          groupContinuesMessage: messageLocalization.format(
            "dxDataGrid-groupContinuesMessage"
          ),
          groupContinuedMessage: messageLocalization.format(
            "dxDataGrid-groupContinuedMessage"
          ),
          groupByThisColumn: messageLocalization.format(
      ...
      groupPanel: {
        visible: false,
        emptyPanelText: messageLocalization.format(
  ...
  extenders: {
    controllers: {
      ...
      columns: (Base: ModuleType<ColumnsController>) =>
        class GroupingColumnsExtender extends Base {
          public _getExpandColumnOptions() {
            const options = (super._getExpandColumnOptions as any).apply(
              this,
              arguments as any
            );
      ...
      editing: (Base: ModuleType<EditingController>) =>
